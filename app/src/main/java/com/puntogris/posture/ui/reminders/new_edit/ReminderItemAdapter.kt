...
class ReminderItemAdapter(
    ...
    private val items = listOf(
        ...
        ReminderUi.Item.Interval(
            ItemData(
                R.string.interval_required,
                context.getString(R.string.create_reminder_interval_summary)
            )
        ),
        ReminderUi.Item.Days(
            ItemData(
                R.string.days_required,
                context.getString(R.string.create_reminder_days_summary)
            )
        ),
        ReminderUi.Item.Start(
            ItemData(
                R.string.start_time_required,
                context.getString(R.string.create_reminder_start_summary)
            )
        ),
        ReminderUi.Item.End(
            ItemData(
                R.string.end_time_required,
                context.getString(R.string.create_reminder_end_summary)
            )
        ),
    ...
    fun updateConfigData(reminder: Reminder) {
        ...
        if (reminder.timeInterval != 0) (items[2] as ReminderUi.Item).itemData.description =
            reminder.timeIntervalSummary()
        if (reminder.alarmDays.isNotEmpty()) (items[3] as ReminderUi.Item).itemData.description =
            reminder.alarmDaysSummary(days)
        if (reminder.startTime != -1) (items[4] as ReminderUi.Item).itemData.description =
            minutesFromMidnightToHourlyTime(reminder.startTime)
        if (reminder.endTime != -1) (items[5] as ReminderUi.Item).itemData.description =
            minutesFromMidnightToHourlyTime(reminder.endTime)
        (items[6] as ReminderUi.Item).itemData.description =
            if (reminder.soundName.isBlank()) context.getString(R.string.disabled) else reminder.soundName
        (items[7] as ReminderUi.Item).itemData.description =
            context.getString(vibrationPatterns[reminder.vibrationPattern])
